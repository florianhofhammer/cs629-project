...
MINIRV32_DECORATE int32_t MiniRV32IMAStep(struct MiniRV32IMAState* state,
                                          uint8_t* image, uint32_t elapsedUs,
                                          int count);
#endif
        ...
        for (int icount = 0; icount < count; icount++) {
            ...
            if (ofs_pc >= MINI_RV32_RAM_SIZE) {
                ...
                switch (ir & 0x7f) {
                        ...
                        if (rsval >= MINI_RV32_RAM_SIZE - 3) {
                                ...
                                && rsval < 0x12000000)  // UART, CLNT
                            {
                                if (rsval
                                    ...
                                    MINIRV32_HANDLE_MEM_LOAD_CONTROL(rsval,
                                                                     rval);
                            } else {
                                trap = (5 + 1);
                                rval = rsval;
                        ...
                        if (addy >= MINI_RV32_RAM_SIZE - 3) {
                            ...
                            if (addy >= 0x10000000 && addy < 0x12000000) {
                                                              ...
                                                              // poweroff, etc.)
                                {
                                    ...
                                    MINIRV32_HANDLE_MEM_STORE_CONTROL(addy,
                                                                      rs2);
                            } else {
                     ...
                     // libgcc support 64-bit math.
                                ...
                                case 4:
                                    if (rs2 == 0)
                                        ...
                                        rval = ((int32_t)rs1 == INT32_MIN
                                                && (int32_t)rs2 == -1)
                                                   ? (uint32_t)rs1
                                                   : (uint32_t)((int32_t)rs1
                                                                / (int32_t)rs2);
                                    break;  // DIV
                                case 5:
                                    if (rs2 == 0)
                            ...
                            switch ((ir >> 12)
                                    & 7)  // These could be either op-immediate
                                          // or op commands.  Be careful.
                            {
                                case 0:
                    ...
                    case 0x0f:     // 0b0001111
                        rdid = 0;  // fencetype = (ir >> 12) & 0b111; We ignore
                                   // fences in this impl.
                        break;
                    case 0x73:  // Zifencei+Zicsr  (0b1110011)
                    {
                        ...
                        if ((microop & 3))  // It's a Zicsr function.
                        {
                            ...
                            switch (csrno) {
                                ...
                                // case 0xf12: rval = 0x00000000; break;
                                // //marchid case 0xf13: rval = 0x00000000;
                                // break; //mimpid case 0xf14: rval =
                                // 0x00000000; break; //mhartid
                                default:
                        ...
                        } else if (microop == 0x0)  // "SYSTEM" 0b000
                        {
                            ...
                            if (csrno == 0x105)  // WFI (Wait for interrupts)
                            {
                                ...
                                CSR(extraflags) |= 4;  // Infor environment we
                                                       // want to go to sleep.
                                SETCSR(pc, pc + 4);
                                return 1;
                            } else if (((csrno & 0xff) == 0x02))  // MRET
                            {
                                ...
                                //  Should also update mstatus to reflect
                                //  correct mode.
                                ...
                                switch (csrno) {
                                    case 0:
                                        trap = (CSR(extraflags) & 3) ? (11 + 1)
                                                                     : (8 + 1);
                                        break;  // ECALL; 8 = "Environment call
                                                // from U-mode"; 11 =
                                                // "Environment call from
                                                // M-mode"
                                    case 1:
                                        trap = (3 + 1);
                    ...
                    case 0x2f:  // RV32A (0b00101111)
                    {
                        ...
                        // We don't implement load/store from UART or CLNT with
                        // RV32A here.
                        ...
                        if (rs1 >= MINI_RV32_RAM_SIZE - 3) {
                            ...
                            switch (irmid) {
                                case 2:  // LR.W (0b00010)
                                    dowrite         = 0;
                                    CSR(extraflags) = (CSR(extraflags) & 0x07)
                                                      | (rs1 << 3);
                                    break;
                                case 3:  // SC.W (0b00011) (Make sure we have a
                                         // slot, and, it's valid)
                                    rval =
                                        (CSR(extraflags) >> 3
                                         != (rs1
                                             & 0x1fffffff));  // Validate that
                                                              // our reservation
                                                              // slot is OK.
                                    dowrite =
